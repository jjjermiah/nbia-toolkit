...
class AbstractListModel(BaseModel, Generic[M]):
	...
	def filter(self, condition: Optional[Callable[[M], bool]] = None) -> "AbstractListModel[M]":
		"""
	...
	def __getitem__(self, index: Union[int, str]) -> M:
		...
		raise TypeError("Index must be an integer or a string representing the key value.")
